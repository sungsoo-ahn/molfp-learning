...
def main():
    ...
    parser.add_argument(
        "--split", type=str, default="scaffold", help="random or scaffold or random_scaffold"
    )
    parser.add_argument(
        "--num_workers", type=int, default=4, help="number of workers for dataset loading"
    )
    ...
    if args.split == "scaffold":
        smiles_list = pd.read_csv("dataset/" + args.dataset + "/processed/smiles.csv", header=None)[
            0
        ].tolist()
    ...
    neptune.append_tag(args.input_model_file)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        neptune.log_metric("val/rocauc", val_acc)
        neptune.log_metric("val/best_rocauc", best_val_acc)
        neptune.log_metric("test/rocauc", test_acc)
        neptune.log_metric("test/best_rocauc", best_test_acc)

    with open("result.csv", "a") as f:
        f.write(f"{args.dataset}, {args.input_model_file}, {best_val_acc}, {best_test_acc}\n")
